...
delay = 8
##Input (Add all post categories in this CSV file)
input_folder = 'Input'
...
main_list = {}
target_text = ''.join(re.findall(r'[\w]+', "it's ok to contact this poster with services or other commercial interests")).lower()
...
def Listing_Parser(Url):
    ...
    if res.status_code == 200:
        page_select = Selector(res.text)
        return page_select.xpath('////li[@class="result-row"]/a/@href').getall()
    return []
...
if __name__ == '__main__':
    ...
    main_df = main_df[main_df['timestamp'] >= time_stamp-weekInSeconds]
        ...
        with Pool() as pool:
            ...
            if urls:
                ...
                for m_url in main_list:
                    main_df = pd.concat([main_df, pd.DataFrame(columns=['timestamp', 'url', 'isMatch'], data=[[time_stamp, m_url, main_list[m_url]]])])
                    if main_list[m_url]:
